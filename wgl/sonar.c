...
static GLfloat sweep_size = 0.3;
static GLfloat font_size = 12;
static Bool resolve_p = True;
static Bool times_p = True;
static Bool wobble_p = True;
static Bool debug_p = False;

static char *team_a_name = "F18";
static char *team_b_name = "MIG";
static int team_a_count = 4;
static int team_b_count = 4;
static int ping_timeout = 3000;
static char *ping_arg = "default";
...
sonar_copy_bogie (sonar_sensor_data *ssd, const sonar_bogie *b)
{
  ...
  b2->name = _strdup (b->name);
  b2->desc = b->desc ? _strdup (b->desc) : 0;
  b2->r    = b->r;
  b2->th   = b->th;
  b2->ttl  = b->ttl;
...
draw_sonar (ModeInfo *mi)
{
  ...
  switch (sp->state) {
  case MSG_:			/* Frame 1: get "Resolving Hosts" on screen. */
