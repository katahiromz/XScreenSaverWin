...
static char *molecule_str = "(default)";
static char *do_spin = "XYZ";
static Bool do_wander = False;
static Bool do_titles = True;
static Bool do_labels = True;
static Bool do_atoms = True;
static Bool do_bonds = True;
static Bool do_shells = True;
static Bool do_bbox = False;
static Bool verbose_p = False;
static GLfloat shell_alpha = 0.3;
  ...
  if (molecule_str && *molecule_str && 
      strcmp(molecule_str, "(default)"))	/* try external PDB files */
    {
      ...
      */
      ...
      if (GetFileAttributesA(molecule_str) != 0xFFFFFFFF &&
          (GetFileAttributesA(molecule_str) & FILE_ATTRIBUTE_DIRECTORY))
        {
          char buf [255];
		  HANDLE hFind;
		  ...
		  do
            {
              ...
              if (L > 4 && !strcasecmp (find.cFileName + L - 4, ".pdb"))
                {
                  char *fn;
...
#if 1
  mc->no_label_threshold = noLabelThreshold;
  mc->wireframe_threshold = wireframeThreshold;
#else
...
pick_new_molecule (ModeInfo *mi, time_t last)
{
  ...
  if (verbose_p)
    {
      char *name = _strdup (mc->molecules[mc->which].label);
      char *s = strpbrk (name, "\r\n");
      if (s) *s = 0;
