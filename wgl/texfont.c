...
bitmap_to_texture (Display *dpy, Pixmap p, Visual *visual, int *wP, int *hP)
{
  ...
  XImage *image = XGetImage (dpy, p, 0, 0, ow, oh, ~0L, RGBAPixmap_);
...
load_texture_font (Display *dpy, char *res)
{
  ...
  char *font = _strdup(res);
  const char *def1 = "System 30";
  const char *def2 = "System 20";
  const char *def3 = "fixed";
  XFontStruct *f;
  int which;
  GLint old_texture = 0;
  ...
  for (which = 0; which < data->ntextures; which++)
...
#if 1
    {
...
#if 1
            int lbearing, width, ascent;
            {
                SIZE siz;
                TEXTMETRIC tm;
                HDC hdc = XCreateDrawableDC_(dpy, p);
                HGDIOBJ hFontOld = SelectObject(hdc, gc->font);
                GetTextExtentPoint32A(hdc, &c, 1, &siz);
                GetTextMetrics(hdc, &tm);
                lbearing = tm.tmOverhang;
                SelectObject(hdc, hFontOld);
                XDeleteDrawableDC_(dpy, p, hdc);
                width = siz.cx;
                ascent = tm.tmAscent;
            }
#else
...
#if 0  /* debugging: write the bitmap to a pgm file */
      {
        ...
        sprintf (file, "%02d.pgm", which);
        //image = XGetImage (dpy, p, 0, 0, w, h, ~0L, ZPixmap);
        image = XGetImage (dpy, p, 0, 0, w, h, ~0L, RGBAPixmap_);
        f = fopen (file, "w");
        fprintf (f, "P5\n%d %d\n255\n", w, h);
        for (y = 0; y < h; y++)
          for (x = 0; x < w; x++) {
...
static int get_lbearing(Font font, char c)
{
    HDC hdc = CreateCompatibleDC(NULL);
    HGDIOBJ hFontOld = SelectObject(hdc, font);
    int n = GetTextCharacterExtra(hdc);
    SelectObject(hdc, hFontOld);
    DeleteDC(hdc);
    return 0;
}

static int get_rbearing(Font font, char c)
{
    SIZE siz;
    HDC hdc = CreateCompatibleDC(NULL);
    HGDIOBJ hFontOld = SelectObject(hdc, font);
...
static int get_ascent(Font font, char c)
{
    TEXTMETRIC tm;
    HDC hdc = CreateCompatibleDC(NULL);
    HGDIOBJ hFontOld = SelectObject(hdc, font);
...
static int get_descent(Font font, char c)
{
    TEXTMETRIC tm;
    HDC hdc = CreateCompatibleDC(NULL);
    HGDIOBJ hFontOld = SelectObject(hdc, font);
...
static int get_cwidth(Font font, char c)
{
    SIZE siz;
    HDC hdc = CreateCompatibleDC(NULL);
    HGDIOBJ hFontOld = SelectObject(hdc, font);
...
# endif /* DO_SUBSCRIPTS */
      else
        {
           ...
           */
#if 1
          int lbearing = get_lbearing(f->fid, c);
          int rbearing = get_rbearing(f->fid, c);
          int ascent = get_ascent(f->fid, c);
          int descent = get_descent(f->fid, c);
          int cwidth = get_cwidth(f->fid, c);
#else
