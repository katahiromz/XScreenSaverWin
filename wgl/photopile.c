 ...
 */
static GLfloat scale = 0.4;        /* Scale factor for loading images. */
static GLfloat max_tilt = 50;     /* Maximum angle from vertical. */
static GLfloat speed = 1.0;        /* Animation speed scale factor. */
static int duration = 5;         /* Reload images after this long. */
static Bool mipmap_p = True;        /* Use mipmaps instead of single textures. */
static Bool titles_p = False;        /* Display image titles. */
static Bool polaroid_p = True;      /* Use instant-film look for images. */
static Bool clip_p = True;          /* Clip images instead of scaling for -polaroid. */
static Bool shadows_p = True;       /* Draw drop shadows. */
                 ...
                 void *closure)
{
  ...
  if (frame->title)
    free (frame->title);
  frame->title = (filename ? _strdup (filename) : 0);
