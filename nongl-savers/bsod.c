...
Bool doMacX = True;
Bool doSCO = True;
Bool doAtari = False;   /* boring */
Bool doBSD = False; /* boring */
Bool doLinux = True;
Bool doSparcLinux = False;  /* boring */
Bool doHPPALinux = True;
Bool doBlitDamage = True;
Bool doSolaris = True;
Bool doHPUX = True;
Bool doTru64 = True;
Bool doApple2 = True;
...
INFO_ENTRY2 entries2[] =
{
    ...
    {"BSD", &doBSD},
    {"Linux", &doLinux},
    {"SparcLinux", &doSparcLinux},
    {"HPPALinux", &doHPPALinux},
    {"BlitDamage", &doBlitDamage},
    {"Solaris", &doSolaris},
    {"HPUX", &doHPUX},
    {"Tru64", &doTru64},
    {"Apple2", &doApple2},
...
Bool load_info_entry2(const char *name)
{
    size_t i, size;
    size = sizeof(entries2) / sizeof(entries2[0]);
    for (i = 0; i < size; i++)
    {
        if (lstrcmpiA(entries2[i].name, name) == 0)
            return *entries2[i].value;
    }
    return False;
}
...
static INFOENTRY entries[] =
{
    ...
    {"macsbug", "font", "Courier 12"},
    {"macsbug", "bigFont", "Courier 30"},
    ...
    {"macx", "textBground", "Black"},
    {"macx", "background", "#888888"},
    {"macdisk", "font", "Courier 25"},
    {"macdisk", "bigFont", "Courier 35"},
    {"sco", "font", "Courier 12"},
    ...
    {"hppalinux", "bigFont", "Courier 40"},
    {"hppalinux", "foreground", "White"},
    {"hppalinux", "background", "Black"},
    {"sparcalinux", "bigFont", "Courier 40"},
    {"sparcalinux", "foreground", "White"},
    {"sparcalinux", "background", "Black"},
    {"bsd", "font", "Courier 20"},
    {"bsd", "bigFont", "Courier 30"},
    {"bsd", "bigFont2", "Courier 50"},
    {"bsd", "foreground", "#c0c0c0"},
    {"bsd", "background", "Black"},
    {"solaris", "font", "Courier 15"},
    {"solaris", "foreground", "Black"},
    {"solaris", "background", "White"},
    {"hpux", "bigFont", "Courier 50"},
    {"hpux", "foreground", "White"},
    {"hpux", "background", "Black"},
    {"os390", "bigFont", "Courier 50"},
    {"os390", "foreground", "Black"},
    {"os390", "background", "Red"},
    {"tru64", "bigFont", "Courier 50"},
    {"tru64", "foreground", "White"},
    {"tru64", "background", "#0000AA"},
    {"vms", "bigFont", "Courier 50"},
    {"vms", "foreground", "White"},
    {"vms", "background", "Black"},
    {"msdos", "bigFont", "Courier 50"},
...
char *load_info_entry(const char *os, const char *name)
{
    size_t i, size;
    size = sizeof(entries) / sizeof(entries[0]);
    for (i = 0; i < size; i++)
    {
        if (lstrcmpiA(entries[i].os, os) == 0 &&
            lstrcmpiA(entries[i].name, name) == 0)
        {
            return _strdup(entries[i].value);
        }
    }
    return NULL;
}
...
char *font = "Courier 10";
char *font2 = "Courier 50";
char *bigFont = "Courier 50";
char *bigFont2 = "Courier 60";

char *foreground = "White";
char *background = "Black";
Bool dontClearRoot = True;
...
static argtype vars[] = 
{
    {&delay, "delay", NULL, "30", t_Int},
    {&debug, "debug", NULL, "False", t_Bool},
    {&doOnly, "doOnly", NULL, "", t_String},
    ...
    {&font, "font", NULL, "Courier 10", t_String},
    {&font2, "font2", NULL, "Courier 20", t_String},
    {&bigFont, "bigFont", NULL, "Courier 40", t_String},
    {&bigFont2, "bigFont2", NULL, "Courier 40", t_String},
...
#define BSOD_TEXT(bst,align,string) do { \
  ...
  (bst)->queue[(bst)->pos].arg1 = (void *) _strdup (__extension__ (string)); \
  ...
  if (! bst->font)
    {
        ...
        bst->font = XLoadQueryFont (dpy,lf.lfFaceName);
    }
  if (! bst->font)
...
#if 1
    {
        const char *fg = load_info_entry(name, "foreground");
        const char *bg = load_info_entry(name, "background");
        if (fg)
            bst->fg = gcv.foreground = load_color(dpy, bst->xgwa.colormap, fg);
        else
            bst->fg = gcv.foreground = 255;
        if (bg)
            bst->bg = gcv.background = load_color(dpy, bst->xgwa.colormap, bg);
        else
            bst->bg = gcv.background = 0;
    }
#else
               ...
               int pix_w, int pix_h)
{
  ...
  XImage *i1 = XGetImage(dpy, pixmap, 0, 0, pix_w, pix_h, ~0L, RGBAPixmap_);
  XImage *i2 = XCreateImage(dpy, visual, depth, RGBAPixmap_, 0, 0,
                pix_w*2, pix_h*2, 8, 0);
...
#if 1
    char *value = load_info_entry("windowslh", "background2");
	unsigned long bg2 = value ? load_color(dpy, bst->xgwa.colormap, value) : 0;
#else
...
amiga (Display *dpy, Window window)
{
  ...
  int lw = 10;
#if 1
  const char *value = load_info_entry("amiga", "background2");
  unsigned long bg2 = value ? load_color(dpy, bst->xgwa.colormap, value) : 0;
#else
  unsigned long bg2 = get_pixel_resource (dpy, bst->xgwa.colormap,
                                          "amiga.background2",
...
macsbug (Display *dpy, Window window)
{
  ...
  unsigned long bg = bst->bg;
#if 1
    const char *value = load_info_entry("macsbug", "borderColor");
	unsigned long bc = value ? load_color(dpy, bst->xgwa.colormap, value) : 0;
#else
...
macx_10_0 (Display *dpy, Window window)
{
  ...
  XClearWindow (dpy, window);
#if 1
    value = load_info_entry("macx", "textForeground");
	if (value)
		XSetForeground (dpy, bst->gc,
						load_color(dpy, bst->xgwa.colormap, value));
    value = load_info_entry("macx", "textBackground");
	if (value)
...
mac_diskfail (Display *dpy, Window window)
{
  ...
  unsigned long bg = bst->bg;
#if 1
    const char *value = load_info_entry("macx", "background");
	unsigned long bg2 = value ? load_color(dpy, bst->xgwa.colormap, value) : 0;
#else
  unsigned long bg2 = get_pixel_resource (dpy, bst->xgwa.colormap,
                                          "macx.background",
  ...
  if (i > 0)
    {
      ...
      while (--i > 0)
        {
          sprintf (buf, " %d", -(int)(random() % 0xFFF));
          BSOD_TEXT (bst, LEFT, buf);
          BSOD_PAUSE (bst, 1000);
        }
      BSOD_TEXT (bst, LEFT, "\nFix? yes\n\n");
...
# endif /* !(defined (__GNUC__) && defined (__VERSION__)) */
  ...
  /* Insert current host name into banner on line 2 */
  {
    ...
    sprintf (ss, linux_panic[1].string, 
          release, sysname, gccversion, version);
...
# endif /* !HAVE_UNAME */
  ...
  args[1] = _strdup (args[0]);
  args[2] = _strdup (args[0]); args[2][i] = '0' + ids[1];
  args[3] = _strdup (args[0]); args[3][i] = '0' + ids[2];

  args[4] = _strdup (args[1]);
  args[5] = _strdup (args[2]);
  args[6] = _strdup (args[3]);
  ...
  if (i > 0)
    {
      int j;
#if 1
      const char *value = load_info_entry("atm", "background");
	  if (value)
		  XSetForeground (dpy, bst->gc,
						  load_color(dpy, bst->xgwa.colormap, value));
#else
          ...
          for (i = 0; i < 200; i++)
            {
              ...
              if (load_info_entry2(all_modes[new_mode].name))
                {
                  dst->which = new_mode;
                  break;
                }
