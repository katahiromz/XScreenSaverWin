...
/*  XSelectInput (dpy, window, st->xgwa.your_event_mask | ExposureMask);*/
  ...
  st->inc = growBy;
  st->spacing = spacing;
  st->border_size = borderSize;
  st->fg_color = load_color(st->dpy, st->xgwa.colormap, foreground);
  st->bg_color = load_color(st->dpy, st->xgwa.colormap, background);
#else
