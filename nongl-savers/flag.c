...
static argtype vars[] = 
{
    {&bitmap, "bitmap", NULL, "", t_String},
    {&font, "font", NULL, DEF_FONT, t_String},
    {&text_, "text", NULL, "", t_String},
};
...
make_flag_bits(ModeInfo *mi)
{
  ...
  char *bitmap_name = _strdup(bitmap);
  char *text = _strdup(text_);
  ...
  if ((!bitmap_name || !*bitmap_name) && (!text || !*text))
	{
	  if (random() & 1)
		{
		  free(bitmap_name);
		  bitmap_name = _strdup("(default)");
		}
	  else
		{
		  free(text);
		  text = _strdup("(default)");
		}
	}
