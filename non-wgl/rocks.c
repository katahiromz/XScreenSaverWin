...
static argtype vars[] = 
{
    ...
    {&foreground, "foreground", NULL, "#E9967A", t_String},
    {&colors, "colors", NULL, "5", t_Int},
    {&count, "count", NULL, "100", t_Int},
...
rocks_init (Display *d, Window w)
{
  ...
  if (mono_p)
    {
      ...
      unsigned int fg = load_color(st->dpy, cmap, foreground);
  ...
  //sst->threed = get_boolean_resource(st->dpy, "use3d", "Boolean");
  st->threed = use3d;
  if (st->threed)
    {
      gcv.background = bg;
      //gcv.foreground = get_pixel_resource (st->dpy, cmap, "left3d", "Foreground");
      gcv.foreground = load_color(st->dpy, cmap, left3d);
      ...
      gcv.foreground = load_color(st->dpy, cmap, right3d);
