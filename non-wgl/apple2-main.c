...
char *mode = "random";
int duration = 60;
char *program = "xscreensaver-text --cols 40";
Bool fast = False;
                 ...
                 void *closure)
{
  ...
  mine->img_filename = (name ? _strdup (name) : 0);
                                 ...
                                 double *next_actiontime)
{
  ...
  case 30:
    ...
    if (mine->img_filename) {
      ...
      basename = tmp = _strdup (mine->img_filename);
      while (1)
        {
...
apple2_init (Display *dpy, Window window)
{
  ...
  global_program = _strdup(program);
  global_fast_p = fast;
