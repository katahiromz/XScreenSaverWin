...
static argtype vars[] = 
{
    {&observer_z_, "observer_z", NULL, "3.0", t_Float},
    {&delay_, "delay", NULL, "10000", t_Int},
    ...
    {&color0, "color0", NULL, "magenta", t_String},
    {&color1, "color1", NULL, "yellow", t_String},
    {&color2, "color2", NULL, "#FF9300", t_String},
    {&color3, "color3", NULL, "#FF0093", t_String},
    {&color4, "color4", NULL, "green", t_String},
    {&color5, "color5", NULL, "#8080FF", t_String},
    {&color6, "color6", NULL, "#00D0FF", t_String},
    {&color7, "color7", NULL, "#00FFD0", t_String},
};
...
hypercube_init (Display *dpy, Window win)
{
  ...
  if (mono_p)
  ...
  else
    {
	  ...
	  fg_pixel = load_color(dpy, cmap, color0);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[0] = color_gc;

	  fg_pixel = load_color(dpy, cmap, color1);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[1] = color_gc;

	  fg_pixel = load_color(dpy, cmap, color2);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[2] = color_gc;

	  fg_pixel = load_color(dpy, cmap, color3);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[3] = color_gc;

	  fg_pixel = load_color(dpy, cmap, color4);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[4] = color_gc;

	  fg_pixel = load_color(dpy, cmap, color5);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[5] = color_gc;

	  fg_pixel = load_color(dpy, cmap, color6);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[6] = color_gc;

	  fg_pixel = load_color(dpy, cmap, color7);
	  gcv.foreground = fg_pixel;
	  color_gc = XCreateGC (dpy, win, GCForeground|GCFunction, &gcv);
	  hs->hs_color_gcs[7] = color_gc;
	  }
#else
      for (col = 0; col < 8; col++)
	{
