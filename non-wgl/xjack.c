...
char *background = "#FFF0B4";
char *foreground = "#000000";
char *font = "Courier New 120";
int delay = 50000;

static argtype vars[] = 
{
    {&background, "background", NULL, "#FFF0B4", t_String},
    {&foreground, "foreground", NULL, "#000000", t_String},
    {&font, "font", NULL, "Courier New 120", t_String},
                 ...
                 unsigned int w, unsigned int h)
{
  ...
  XClearWindow2_(st->dpy, st->window, st->gc);
}
...
xjack_init (Display *dpy, Window window)
{
  ...
  if (st->xgwa.width <= 480)
    ...
    fontname = "Courier New 90";

  st->font = XLoadQueryFont (st->dpy, fontname);

  if (!st->font)
    st->font = XLoadQueryFont (st->dpy, "System 120");
  if (!st->font)
    st->font = XLoadQueryFont (st->dpy, "Terminal 120");
#if 0
  if (!st->font)
  ...
  st->char_width = 
    (st->font->per_char
     ...
     ? st->font->per_char['w'-st->font->min_char_or_byte2].rbearing
...
#if 1
      XClearArea2_(st->dpy, st->window, st->gc,
