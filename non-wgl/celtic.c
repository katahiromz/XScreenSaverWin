...
static argtype vars[] = 
{
    ...
    {&ncolors, "ncolors", NULL, "20", t_Int},
    {&delay, "delay", NULL, "10000", t_Int},
    {&delay2, "delay2", NULL, "5", t_Int},
    {&showGraph, "showGraph", NULL, "False", t_Bool},
};
...
{
  Array new;
  new=(Array)calloc(1,sizeof(struct array));
  assert(new);
  new->nb_elements=0;
...
{
  Node new;
  new = (Node)calloc(1,sizeof(struct node));
  assert(new);
  new->x=x;
  new->y=y;
...
{
  Edge new; 
  new = (Edge)calloc(1,sizeof(struct edge));
  assert(new);
  new->node1=n1;
  new->node2=n2;
...
{
  Graph new;
  new = (Graph)calloc(1,sizeof(struct graph));
  assert(new);
     ...
     /* make a simple grid graph, with edges present or absent randomly */
{
  ...
  /* generate nodes */
  grid=(Node*)calloc(1+nbp*nbo,sizeof(Node));
  assert(grid);
  g=graph_new(st);
  assert(g);
     ...
     /* make a simple grid graph */
{
  ...
  Node *grid;
  grid=(Node*)calloc(nbrow*nbcol,sizeof(Node));
  assert(grid);
  g=graph_new(st);
  assert(g);
                                 ...
                                 int xmin, int ymin, int width, int height, int edge_size)
{
  ...
  grid=(Node*)calloc((nsteps+1)*(nsteps+1),sizeof(Node));
  assert(grid);
  g=graph_new(st);
  assert(g);
      ...
      */
{
  ...
  grid=(Node*)calloc(5*nbrow*nbcol,sizeof(Node));
  assert(grid);
  g=graph_new(st);
  assert(g);
...
{
  Pattern new;
  new=(Pattern)calloc(1,sizeof(struct pattern));
  assert(new);
  new->shape1=shape1;
  new->shape2=shape2;
  new->graph=g;
...
//                                             "background", "Background");
	st->colors[0].pixel = load_color(st->dpy, st->xgwa.colormap, foreground);
	st->colors[1].pixel = load_color(st->dpy, st->xgwa.colormap, background);
      }
    }
