...
static argtype vars[] = 
{
    ...
    {&subdelay, "subdelay", NULL, "20000", t_Int},
    {&layers, "layers", NULL, "2", t_Int},
    {&alwaysfinish, "alwaysfinish", NULL, "False", t_Bool},
};
...
init_tsg (struct state *st)
{
  ...
  gcv.foreground = st->default_fg_pixel =
    load_color(st->dpy, cmap, foreground);
...
#if 1
  st->long_delay = delay;
  st->sub_sleep_time = subdelay;
  st->num_layers = layers;
  st->always_finish_p = alwaysfinish;
#else
