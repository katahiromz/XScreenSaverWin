...
int minCircles = 2;
int maxCircles = 10;
float minSpeed = 0.003;
float maxSpeed = 0.005;
int harmonics = 8;
float timestep = 1.0;
float timestepCoarseFactor = 1.0;
float divisorPoisson = 0.4;
float sizeFactorMin = 1.05;
float sizeFactorMax = 2.05;

static argtype vars[] = 
{
    {&background, "background", NULL, "black", t_String},
    {&foreground, "foreground", NULL, "white", t_String},
    {&colors, "colors", NULL, "100", t_Int},
    {&color0, "color0", NULL, "red", t_String},
    {&delay, "delay", NULL, "20000", t_Int},
    {&holdtime, "holdtime", NULL, "2", t_Int},
    {&lineWidth, "lineWidth", NULL, "4", t_Int},
    {&minCircles, "minCircles", NULL, "2", t_Int},
    {&maxCircles, "maxCircles", NULL, "10", t_Int},
    {&minSpeed, "minSpeed", NULL, "0.003", t_Float},
    {&maxSpeed, "maxSpeed", NULL, "0.005", t_Float},
    {&harmonics, "harmonics", NULL, "8", t_Int},
    {&timestep, "timestep", NULL, "1.0", t_Float},
    {&timestepCoarseFactor, "timestepCoarseFactor", NULL, "1.0", t_Float},
    {&divisorPoisson, "divisorPoisson", NULL, "0.4", t_Float},
    {&sizeFactorMin, "sizeFactorMin", NULL, "1.05", t_Float},
    {&sizeFactorMax, "sizeFactorMax", NULL, "2.05", t_Float},
};
...
colour_init(struct state *st, XWindowAttributes *pxgwa)
{
  ...
  st->bg = load_color(st->dpy, st->cmap, background);
...
/*  if (mono_p)*/
    ...
    fg = load_color(st->dpy, st->cmap, foreground);
/* WTF?
else
    //fg = st->bg ^ get_pixel_resource (st->dpy, st->cmap, ("color0"), "Foreground"); 
    fg = st->bg ^ load_color(st->dpy, st->cmap, color0);
*/
  gcv.foreground = fg;
...
epicycle_init (Display *disp, Window win)
{
  ...
  st->timestep_coarse = st->timestep * timestepCoarseFactor;
#else
  ...
  st->minCircles = minCircles;
  st->maxCircles = maxCircles;
#else
