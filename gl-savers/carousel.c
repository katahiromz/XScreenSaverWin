...
static int duration = 20;	    /* reload images after this long */
static Bool mipmap_p = True;	    /* Use mipmaps instead of single textures. */
static Bool titles_p = True;	    /* Display image titles. */
static Bool zoom_p = True;	    /* Throb the images in and out as they spin. */
static char *tilt_str = "XY";
static Bool tilt_x_p = 0;	    /* Tilt axis towards the viewer */
static Bool tilt_y_p = 0;	    /* Tilt axis side to side */
                 ...
                 void *closure)
{
  ...
  frame->loading.title = (filename ? _strdup (filename) : 0);
