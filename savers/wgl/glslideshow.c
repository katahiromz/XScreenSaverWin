 ...
 */
static int fade_seconds = 2;    /* Duration in seconds of fade transitions.
                               If 0, jump-cut instead of fading. */
static int pan_seconds = 6;     /* Duration of each pan through an image. */
static int image_seconds = 30;   /* How many seconds until loading a new image. */
static int zoom = 75;            /* How far in to zoom when panning, in percent of
                               image size: that is, 75 means "when zoomed all
                               the way in, 75% of the image will be visible."
                             */
static int fps_cutoff = 5;      /* If the frame-rate falls below this, turn off
                               zooming.*/
static Bool letterbox_p = True;    /* When a loaded image is not the same aspect
                               ratio as the window, whether to display black
                               bars.
                             */
static Bool mipmap_p = True;	    /* Use mipmaps instead of single textures. */
static Bool do_titles = False;	    /* Display image titles. */
static Bool debug_p = False;	    /* Be loud and do weird things. */
                 ...
                 void *closure)
{
  ...
  img->geom = *geom;
  img->title = (filename ? _strdup (filename) : 0);
